...
function announceRoll(
  ...
  if (move) {
    ...
    announcement = t("a11y-action-roll-no-move", "Rolled plus {{statLabel}}.", {
      statLabel: roll.rollLabel,
    });
  }
  ...
  if (roll.matchedNegativeMomentum) {
    announcement += t(
      "a11y-action-roll-total-matched-negative-momentum",
  ...
  const resultLabel = t("weak-hit", "Weak Hit");
  if (roll.result === RollResult.StrongHit) {
    t("strong-hit", "Strong Hit");
  } else if (roll.result === RollResult.Miss) {
    t("miss", "Miss");
  }
  announcement += t(
    "a11y-action-roll-result",
