...
export const themeConfig = (colorPalette: ColorScheme) =>
  defineConfig({
    theme: {
      tokens: {
        colors: {
          ...
          blue: sky,
          purple,
          brand: getBrandVariables(colorPalette),
        },
        fonts: {
        ...
        colors: {
          brand: {
            "gradient-start-color": {
              value:
                colorPalette === ColorScheme.Default
                  ? "{colors.brand.500}"
                  : "{colors.brand.300}",
            },
            "gradient-end-color": {
            ...
            solid: {
              value: {
                _light: colorSchemesRequiringDarkerBackgrounds.includes(
                  colorPalette,
                )
                  ? "{colors.brand.700}"
                  : "{colors.brand.600}",
                _dark: colorSchemesRequiringDarkerBackgrounds.includes(
                  colorPalette,
                )
                  ? "{colors.brand.700}"
                  : "{colors.brand.600}",
...
const colorSchemesRequiringDarkerBackgrounds = [
  ColorScheme.Default,
  ColorScheme.Eidolon,
...
function getBrandVariables(colorPalette: ColorScheme) {
  return {
    50: { value: `{colors.${colorSchemeMap[colorPalette]}.50}` },
    100: { value: `{colors.${colorSchemeMap[colorPalette]}.100}` },
    200: { value: `{colors.${colorSchemeMap[colorPalette]}.200}` },
    300: { value: `{colors.${colorSchemeMap[colorPalette]}.300}` },
    400: { value: `{colors.${colorSchemeMap[colorPalette]}.400}` },
    500: { value: `{colors.${colorSchemeMap[colorPalette]}.500}` },
    600: { value: `{colors.${colorSchemeMap[colorPalette]}.600}` },
    700: { value: `{colors.${colorSchemeMap[colorPalette]}.700}` },
    800: { value: `{colors.${colorSchemeMap[colorPalette]}.800}` },
    900: { value: `{colors.${colorSchemeMap[colorPalette]}.900}` },
    950: { value: `{colors.${colorSchemeMap[colorPalette]}.950}` },
  };
}
